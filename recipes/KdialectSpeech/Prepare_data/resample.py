...
filehandler = logging.FileHandler('resample-{:%Y%m%d%H}.log'.format(datetime.datetime.now()), encoding='utf-8')
...
if __name__ == '__main__':
    ...
    audio_dict_files = [
        # "jj_wrong_samplerate_3.txt",

        "cc_wrong_samplerate_1.txt",
        "cc_wrong_samplerate_2.txt",
        "cc_wrong_samplerate_3.txt",
        
        "jl_wrong_samplerate_1.txt",
        "jl_wrong_samplerate_2.txt",
        "jl_wrong_samplerate_3.txt",

        "gw_wrong_samplerate_1.txt",
        "gw_wrong_samplerate_2.txt",
        "gw_wrong_samplerate_3.txt",

        "gs_wrong_samplerate_1.txt",
        "gs_wrong_samplerate_2.txt",
        "gs_wrong_samplerate_3.txt",
        ]

    for audio_dict_file in audio_dict_files:

        # audio_dict_file = 'jj_wrong_samplerate_3.txt'
        logger.info(f'{audio_dict_file}의 파일들을 변환 시작----- : {datetime.datetime.now()}')
        for file, sr in tqdm(read_audiosample(audio_dict_file).items()):
            y, sr = librosa.load(file, sr=sr)
            resample = librosa.resample(y, sr, SAMPLERATE)
            sf.write(file, resample, SAMPLERATE, format='WAV', endian='LITTLE', subtype='PCM_16')

        logger.info(f'{audio_dict_file}의 파일들을 변환 종료----- : {datetime.datetime.now()}')
