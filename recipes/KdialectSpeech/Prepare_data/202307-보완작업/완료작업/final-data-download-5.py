...
사용법 :
nohup python final-data-download-4.py &> nohup-download-4.out & 
"""
...
# 3
# prefix = '139-1.중·노년층 한국어 방언 데이터 (강원도, 경상도)/07.보완조치/01.보완완료/03.유효성/1. 음성인식모델/평가데이터셋/1. 원천데이터/'

# 1
# prefix = '139-1.중·노년층 한국어 방언 데이터 (강원도, 경상도)/07.보완조치/01.보완완료/03.유효성/1. 음성인식모델/평가데이터셋/2. 라벨링데이터/'

# 4
# prefix = '139-2.중·노년층 한국어 방언 데이터 (충청도, 전라도, 제주도)/07.보완조치/01.보완완료/03.유효성/1. 음성인식모델/평가데이터셋/1. 원천데이터/'

# 2
# prefix = '139-2.중·노년층 한국어 방언 데이터 (충청도, 전라도, 제주도)/07.보완조치/01.보완완료/03.유효성/1. 음성인식모델/평가데이터셋/2. 라벨링데이터/'

# 5
prefix = '139-1.중·노년층 한국어 방언 데이터 (강원도, 경상도)/07.보완조치/01.보완완료/03.유효성/1. 음성인식모델/평가데이터셋/3. test_file_list/'

# 6
# prefix = '139-2.중·노년층 한국어 방언 데이터 (충청도, 전라도, 제주도)/07.보완조치/01.보완완료/03.유효성/1. 음성인식모델/평가데이터셋/3. test_file_list/'
...
with open(error_file_path, 'w') as lf:
    ...
    for key in tqdm(get_s3_object_list(s3, bucket_name, prefix, max_keys)):
        ...
        if Path(key).suffix in ['.json', '.wav', '.csv', '.sh', '.py', '.yaml']:
